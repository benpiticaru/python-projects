...
def make_plot(sample_1,name_1,sample_2,name_2):
    ...
    ax = df.plot.scatter(x='avg_x',y='avg_y',color='black',marker='.')
    df.set_index('avg_x', inplace=True)
    df.trendline.sort_index(ascending=False).plot(ax=ax,color='black')
    title_1 = '{name_1} Average Poly(A) Length'.format(name_1=name_1)
    title_2 = '{name_2} Average Poly(A) Length'.format(name_2=name_2)
    plt.xlabel(title_1) 
    plt.ylabel(title_2)
    plt.title('Scatterplot of Unique Gene Average Poly(A)')
    plt.gca()
    plt.savefig("Poly_A_Scatterplot")
